...
| Preview image | `<img src="…">` pointing at the source's own server | See §5 — **not re-hosted** |
...
### 5.1 What the database stores

An SQLite column `articles.image_url` of type `TEXT` holds a URL string
pointing to the source publisher's own CDN, e.g.
`https://cdn.source-publisher.example/…/hero.jpg`. **No binary image
data, no downloaded copy, no base64-embedded copy.**

### 5.2 What is deployed to GitHub Pages
...
### 5.3 What the visitor's browser does
...
### 5.4 Fallback when the source removes the image
...
### 5.6 Single exception — Instagram card generation
...
| §5.1 | `image_url` is a URL string, not a blob | `admin/lib/schema.ts` — column declared `TEXT` |
| §5.2 | No image bytes in published markdown | `admin/app/api/publish/route.ts` — `image_url: {{ URL }}` in generated YAML |
| §5.3 | Public `<img>` tags point to source URLs | `site/src/index.njk`, `site/src/_includes/post.njk`, `site/src/archive/month.njk` |
| §5.4 | Broken image is hidden, not cached | `onerror="this.style.display='none'"` on each `<img>` |
| §5.5 | OG image points to source URL | `site/src/_includes/base.njk` — `<meta property="og:image" content="{{ image_url }}">` |
| §5.6 | Instagram card is a derivative composite | `admin/lib/instagram-card-og.tsx` — Satori renders a new 1080×1080 PNG |
