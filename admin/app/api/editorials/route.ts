...
export async function GET() {
  const result = await db.execute(
    "SELECT id, slug, status, source_language, title_en, title_nl, title_fr, summary_en, article_emoji, image_filename, post_to_substack, substack_posted_at, publish_date, published_at, audio_generated_at, created_at, updated_at FROM editorials ORDER BY created_at DESC"
  );
